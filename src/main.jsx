      ...
      try {
        await Promise.all([
          insertJobNoteToSupabase({
            ...
            visitId: getCurrentVisitId(changedJob) || null,
            createdBy: currentUser?.id || null
          }),
          insertJobHistoryToSupabase({
            jobId,
...
async function fetchJobsFromSupabase() {
  ...
  let workerNames = {};
  if (jobIds.length) {
    const [attachmentResult, noteResult, historyResult, workerResult] = await Promise.all([
  ...
  return (jobRows || []).map(row => {
    ...
    // A completion submission is stored in job_notes because employees do not
    // have permission to update the main jobs row. Rebuild the current visit's
    // completion evidence here so Admin can see notes and Confirm complete.
    const workerCompletions = { ...(job.workerCompletions || {}) };
    remoteNotes.forEach(note => {
      ...
      const belongsToCurrentVisit = currentVisitId
        ? (note.visit_id === currentVisitId || (!note.visit_id && noteTime >= currentVisitStartedAt))
        : true;
      if (!belongsToCurrentVisit || workerCompletions[note.worker_id]) return;
      workerCompletions[note.worker_id] = {
        visitId: currentVisitId || note.visit_id || null,
    ...
    job = {
      ...
      noteHistory: mergeHistoryEntries(noteHistory, job.noteHistory || []),
