...
import {
  ...
  AlertCircle, CalendarX, Plane, Wrench, RotateCcw, UserMinus, Settings, ZoomIn, ZoomOut, Copy
} from "lucide-react";
...
const STORAGE_KEY = "jobsched-v30-quick-actions-messages";
...
function App() {
  ...
  function sendDemoMessage(job, customText = "") {
    ...
    updateData({ ...data, messages: [message, ...data.messages], jobs: nextJobs });
  ...
  return (
    <div className={activeView === "admin" ? "app admin-mode" : currentUser ? "app nav-mode" : "app"}>
      ...
      {jobMessagesJob && <JobMessagesModal job={jobMessagesJob} messages={data.messages.filter(m=>m.jobId===jobMessagesJob.id)} onClose={()=>setJobMessagesJob(null)} onAction={markMessageActioned}/>}
        ...
        {isStart ? (
          <>
            <div className="card-top">
              ...
              <div className="card-actions"><button onClick={(e)=>{e.stopPropagation(); onEdit();}}><Pencil size={14}/></button><button onClick={(e)=>{e.stopPropagation(); onDelete();}}><Trash2 size={14}/></button></div>
            </div>
            <h3>{job.title}</h3>
