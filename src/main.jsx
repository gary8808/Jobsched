...
function App() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "job_notes" }, refreshJobsAndBookings)
      .on("postgres_changes", { event: "*", schema: "public", table: "job_completion_submissions" }, refreshJobsAndBookings)
      .subscribe();

    // The completion tables may not yet be included in the project's Realtime
    // publication. A lightweight refresh keeps Admin notes and Confirm Complete
    // reliable even when those database events are unavailable.
    const fallbackInterval = window.setInterval(refreshJobsAndBookings, 15000);
    const refreshOnFocus = () => refreshJobsAndBookings();
  ...
  async function saveEmployeeCompletion(jobId, workerId, completion) {
    ...
    if (changedJob && session && supabase) {
      ...
      try {
        ...
        // Reload the server record immediately so the saved completion receives
        // its ledger submission ID and appears in Notes without a page refresh.
        const freshJobs = await fetchJobsFromSupabase();
        setData(current => ({ ...current, jobs: freshJobs }));
...
async function fetchJobsFromSupabase() {
  ...
  if (jobIds.length) {
    const [attachmentResult, noteResult, historyResult, completionResult, workerResult] = await Promise.all([
      ...
      supabase.from("job_notes").select("id,job_id,worker_id,note_text,note_type,visit_id,created_by,created_at").in("job_id", jobIds).order("created_at", { ascending: false }),
  ...
  return (jobRows || []).map(row => {
    ...
    // the current completion in Admin and Employee views.
    const workerCompletions = {};
...
async function insertJobNoteToSupabase({ jobId, workerId, noteText, noteType = "general", visitId = null, createdBy = null }) {
