...
function App() {
    ...
    async function checkSupabase() {
      ...
      try {
        ...
        setSupabaseCheck({
          status: "connected",
          workers: workers || [],
          message: `Connected to Supabase. Workers found: ${(workers || []).length}`
        });
      } catch (error) {
        if (!alive) return;
        setSupabaseCheck({
  ...
  return (
    <div className={view === "admin" ? "app admin-mode" : "app"}>
      ...
      {view === "admin" && <SupabaseTestPanel result={supabaseCheck} />}
...
function SupabaseTestPanel({ result }) {
  ...
  return (
    <section className={`supabase-test-panel ${connected ? "connected" : checking ? "checking" : "error"}`}>
      <div>
        <strong>{connected ? "Supabase connected" : checking ? "Checking Supabase" : "Supabase needs attention"}</strong>
