...
const STORAGE_KEY = "jobsched-v29-supabase-jobs-bookings";
...
function App() {
  ...
  useEffect(() => {
    ...
    async function checkSupabase() {
      ...
      try {
        ...
        if (workers.length || remoteJobs.length) {
          setData(current => ({
            ...current,
        ...
        setJobsSyncMessage(remoteJobs.length ? `Loaded ${remoteJobs.length} job(s) from Supabase.` : "No Supabase jobs found yet. New/edited jobs will save to Supabase.");

        setSupabaseCheck({
          status: "connected",
          workers,
          message: `Authenticated as ${session.user?.email || "user"}. Workers found: ${workers.length}. Jobs found: ${remoteJobs.length}`
        });
      } catch (error) {
        if (!alive) return;
        setSupabaseCheck({
  ...
  function saveCalendarItem(item) {
    if (item.type === "adHoc" || item.type === "travel") {
      ...
      const loggedJob = logJob(job, "Created", item.type === "travel" ? "Travel/accommodation comment added." : "Ad hoc job added.");
      updateData({ ...data, jobs: [loggedJob, ...data.jobs] });
      if (session && supabase) persistJobToSupabase(loggedJob).catch(err => setJobsSyncMessage(`Supabase job sync failed: ${err.message}`));
  ...
  function sendDemoMessage(job, customText = "") {
    ...
    const nextJobs = data.jobs.map(j => {
      ...
      changedJob = logJob({ ...j, appointmentSent: true }, "Message sent", "Demo SMS generated.");
      return changedJob;
    });
    updateData({ ...data, messages: [reply, message, ...data.messages], jobs: nextJobs });
    if (changedJob && session && supabase) persistJobToSupabase(changedJob).catch(err => setJobsSyncMessage(`Supabase job sync failed: ${err.message}`));
    return true;
  }
  ...
  function updateWorkerJobStatus(jobId, workerId, nextStatus) {
    updateJobs(job => {
      ...
      return logJob({ ...job, ...completionPatch, workerStatus: updatedStatus }, "Employee status", `${getWorkerName(data.teamMembers, workerId)} changed status to ${STATUS_META[nextStatus].label}.`);
    });
  }
...
function CalendarJob({ job, workerId, isStart, hasUnreadMessage, onDragStart, onEdit, onDelete, onToggleAppointmentSent, onToggleClientAccepted, onConfirmComplete }) {
  ...
  return (
    <article className={`calendar-job ${defectJob ? "defect-callback" : ""} ${job.category === "Completed" || job.completedConfirmed ? "supervisor-complete" : ""} ${completedByTrade && job.category !== "Completed" ? "trade-complete" : ""} ${job.isAdHoc?"ad-hoc":""} ${job.isTravelComment?"travel-comment":""} ${isStart?"range-start":"range-middle"}`} draggable onDragStart={onDragStart}>
                  ...
                  <article key={job.id} className={`employee-job-card app-job-card ${status === "completed" || job.category === "Completed" || job.completedConfirmed ? "employee-complete" : ""}`}>
                    ...
                    {job.isAdHoc ? (
                      ...
                      <>
                        ...
                        <div className="employee-save-row"><button className="primary" onClick={() => saveCompletion(job)}>Save update</button></div>
                      </>
                    )}
                  </article>
                );
...
function getJobBookingsForSupabase(job) {
  ...
  if (hasPrimaryBooking(job)) {
    (job.assignedTo || []).forEach(workerId => {
      if (workerId && job.startDate && job.endDate) rows.push({ job_id: job.id, worker_id: workerId, start_date: job.startDate, end_date: job.endDate, booking_status: job.category || "scheduled" });
    });
  }
  (job.scheduleBlocks || []).forEach(block => {
    if (block.workerId && block.startDate && block.endDate) rows.push({ id: isUuid(block.id) ? block.id : undefined, job_id: job.id, worker_id: block.workerId, start_date: block.startDate, end_date: block.endDate, booking_status: job.category || "scheduled" });
  });
  return rows;
}
...
async function fetchJobsFromSupabase() {
  ...
  return (jobRows || []).map(row => mapJobFromSupabase(row, bookingRows || []));
}
