...
function parseFastFieldCloseoutText(text){
  ...
  const yesNo=v=>/^yes\b/i.test(cleanPdfField(v))?true:/^no\b/i.test(cleanPdfField(v))?false:null;
  const overallAnswer=yesNo(overall);
  const additionalAnswer=yesNo(additional);
  const cleanFurther=cleanPdfField(further);
  const cleanRequiredTrade=cleanPdfField(requiredTrade);
  const looksLikeQuestion=v=>/^(?:\.|:|-)?\s*(?:please\s+)?(?:state|describe|provide|if\b|is\b|are\b|has\b|what\b)|what'?s left to finish\?|additional works/i.test(cleanPdfField(v));
  const meaningfulFurther=cleanFurther&&!looksLikeQuestion(cleanFurther)?cleanFurther:"";
  const meaningfulRequiredTrade=cleanRequiredTrade&&!looksLikeQuestion(cleanRequiredTrade)?cleanRequiredTrade:"";
  const furtherWorkIdentified=(overallAnswer===false&&Boolean(meaningfulFurther))||additionalAnswer===true;
  return {rawJobNumber:cleanPdfField(rawJob).match(/\d+/)?.[0]||"",normalisedJobNumber:normaliseImportedJobNumber(rawJob),workOrderNumber:cleanPdfField(workOrder).match(/[A-Z]*\s*\d{4,}/i)?.[0]?.replace(/\s/g,"")||cleanPdfField(workOrder),referenceText:reference,submittedByEmail:cleanPdfField(email),submittedAt:submitted,overallJobComplete:overallAnswer,tradeWorkComplete:yesNo(tradeComplete),furtherWorkText:meaningfulFurther,additionalTradeRequired:additionalAnswer,requiredTrade:meaningfulRequiredTrade,supervisorChecked:yesNo(supervisor),furtherWorkIdentified};
}
      ...
      <div className="modal settings-modal">
        ...
        <div className="people-layout">
          <aside className="people-list">
            ...
            {filtered.map(w=><button key={w.id} type="button" draggable={!w.inactive} className={selected?.id===w.id?"active":""} onDragStart={()=>setDragWorkerId(w.id)} onDragOver={e=>{if(!w.inactive)e.preventDefault();}} onDrop={e=>{e.preventDefault();reorderWorkerBefore(dragWorkerId,w.id);setDragWorkerId("");}} onClick={()=>setSelectedId(w.id)}><strong>{w.name||"Unnamed employee"}</strong><span>{w.trade||"No trade"}{w.inactive ? " · archived" : ""}{w.appRole ? ` · ${w.appRole}` : ""}</span></button>)}
