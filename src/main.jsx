...
function normaliseImportedJobNumber(value){return String(value||"").toUpperCase().trim().replace(/^J(?=\d)/,"").replace(/[^0-9]/g,"");}
...
function parseFastFieldCloseoutText(text){
  ...
  const overall=pdfFieldBetween(text,"Is the overall job complete?",["Are your part of the works complete?"]);
  const tradeComplete=pdfFieldBetween(text,"Are your part of the works complete?",["If the overall job is not complete"]);
  const further=pdfFieldBetween(text,"If the overall job is not complete. Please state what's left to finish?",["Is there any additional works to be completed by another trade?"])
    ||pdfFieldBetween(text,"If the overall job is not complete",["Is there any additional works"]);
  const additional=pdfFieldBetween(text,"Is there any additional works to be completed by another trade?",["If additional works are left"]);
  const requiredTrade=pdfFieldBetween(text,"If additional works are left to be completed by other trades, please state if known?",["Has your AIM Construction Group Supervisor"]);
  const supervisor=pdfFieldBetween(text,"Has your AIM Construction Group Supervisor been around to check your works?",["Date Submitted:"]);
  ...
  const yesNo=v=>/^yes\b/i.test(v)?true:/^no\b/i.test(v)?false:null;
  return {rawJobNumber:cleanPdfField(rawJob).match(/\d+/)?.[0]||"",normalisedJobNumber:normaliseImportedJobNumber(rawJob),workOrderNumber:cleanPdfField(workOrder).match(/[A-Z]*\s*\d{4,}/i)?.[0]?.replace(/\s/g,"")||cleanPdfField(workOrder),referenceText:reference,submittedByEmail:cleanPdfField(email),submittedAt:submitted,overallJobComplete:yesNo(overall),tradeWorkComplete:yesNo(tradeComplete),furtherWorkText:further,additionalTradeRequired:yesNo(additional),requiredTrade,supervisorChecked:yesNo(supervisor),furtherWorkIdentified:Boolean(further)||yesNo(additional)===true};
}
...
function FastFieldCloseoutsPage({jobs,onClose}){
  const [rows,setRows]=useState([]);const [query,setQuery]=useState("");const [selected,setSelected]=useState(null);const [jobId,setJobId]=useState("");const [busy,setBusy]=useState(false);const [parsingId,setParsingId]=useState("");
  async function load(autoParse=true){if(!supabase)return;const {data,error}=await supabase.from("fastfield_submissions").select("*").order("received_at",{ascending:false});if(error){alert(`Could not load FastField submissions: ${error.message}`);return;}const loaded=data||[];setRows(loaded);if(autoParse){const pending=loaded.filter(r=>r.pdf_object_path&&(!r.normalised_job_number||r.processing_status==="parsing_pending")).slice(0,5);for(const row of pending)await parsePdf(row,true);}}
  useEffect(()=>{load();},[]);
  async function parsePdf(row,silent=false){if(!row?.pdf_bucket||!row?.pdf_object_path)return;setParsingId(row.id);try{await supabase.from("fastfield_submissions").update({processing_status:"parsing",parse_attempted_at:new Date().toISOString(),updated_at:new Date().toISOString()}).eq("id",row.id);const file=await downloadPrivatePdf(row.pdf_bucket,row.pdf_object_path);const text=await extractTextFromPdf(file);const parsed=parseFastFieldCloseoutText(text);if(!parsed.rawJobNumber&&!parsed.workOrderNumber)throw new Error("The labelled job number and work order could not be read from page 1.");let matchedJob=null;const exact=jobs.filter(j=>normaliseImportedJobNumber(j.jobNumber)===parsed.normalisedJobNumber);const confirmed=parsed.workOrderNumber?exact.filter(j=>String(j.workOrderNumber||"").replace(/\s/g,"")===parsed.workOrderNumber.replace(/\s/g,"")):exact;if(confirmed.length===1)matchedJob=confirmed[0];else if(exact.length===1&&!parsed.workOrderNumber)matchedJob=exact[0];const update={raw_job_number:parsed.rawJobNumber||null,normalised_job_number:parsed.normalisedJobNumber||null,work_order_number:parsed.workOrderNumber||null,reference_text:parsed.referenceText||null,submitted_by_email:parsed.submittedByEmail||null,submitted_at:parseFastFieldSubmittedDate(parsed.submittedAt),overall_job_complete:parsed.overallJobComplete,trade_work_complete:parsed.tradeWorkComplete,further_work_identified:parsed.furtherWorkIdentified,further_work_text:parsed.furtherWorkText||null,additional_trade_required:parsed.additionalTradeRequired,required_trade:parsed.requiredTrade||null,supervisor_checked:parsed.supervisorChecked,processing_status:matchedJob?"parsed_matched":"parsed_unmatched",parse_source:"pdf_page_text",parse_confidence:parsed.rawJobNumber&&parsed.workOrderNumber?1:0.75,parsed_at:new Date().toISOString(),updated_at:new Date().toISOString(),error_message:null};const {error}=await supabase.from("fastfield_submissions").update(update).eq("id",row.id);if(error)throw error;if(matchedJob){const {error:matchError}=await supabase.rpc("aimcg_match_fastfield_submission",{p_submission_id:row.id,p_job_id:matchedJob.id});if(matchError)throw matchError;}await load(false);}catch(error){await supabase.from("fastfield_submissions").update({processing_status:"parse_failed",error_message:error.message,parse_attempted_at:new Date().toISOString(),updated_at:new Date().toISOString()}).eq("id",row.id);if(!silent)alert(error.message);await load(false);}finally{setParsingId("");}}
  ...
  async function openPdf(row){if(!row.pdf_bucket||!row.pdf_object_path)return;const {data,error}=await supabase.storage.from(row.pdf_bucket).createSignedUrl(row.pdf_object_path,120);if(error){alert(error.message);return;}window.open(data.signedUrl,"_blank","noopener,noreferrer");}
  const q=query.trim().toLowerCase();const visible=rows.filter(r=>!q||[r.raw_job_number,r.work_order_number,r.reference_text,r.submitted_by_email,r.match_status,r.pdf_file_name].join(" ").toLowerCase().includes(q));
  return <main className="reports-page"><section className="reports-page-shell"><div className="inventory-header"><div><h1><ClipboardList size={28}/> FastField close-outs</h1><p>PDFs are stored immediately, then the fixed first-page labels are read and matched to AIM jobs. Unrelated reports remain safely unmatched.</p></div><div className="report-actions"><button className="secondary" onClick={()=>load()}><RotateCcw size={15}/> Refresh and parse</button><button className="secondary" onClick={onClose}><ChevronLeft size={17}/> Back</button></div></div><div className="search closeout-search"><Search size={16}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search job number, work order, reference, file or submitter…"/></div><div className="closeout-summary"><span>{rows.filter(r=>r.match_status==="unmatched").length} unmatched</span><span>{rows.filter(r=>r.match_status==="matched").length} matched</span><span>{rows.filter(r=>r.processing_status==="parse_failed").length} need review</span></div><div className="closeout-list">{visible.map(r=><article className={`closeout-row ${r.further_work_identified?"warning":""}`} key={r.id}><div><strong>{r.raw_job_number?`Job ${r.raw_job_number}`:"Job number pending"} · WO {r.work_order_number||"pending"}</strong><span>{r.reference_text||r.pdf_file_name||r.form_name||"FastField close-out"}</span><em>{r.submitted_by_email||r.submitted_by_name||"Submitter pending"} · {formatDateTime(r.submitted_at||r.received_at)}</em><small>Status: {r.match_status} / {r.processing_status}{r.error_message?` · ${r.error_message}`:""}{r.further_work_identified?" · Further work identified":""}</small></div><div className="closeout-actions">{r.pdf_object_path&&<button className="secondary" onClick={()=>openPdf(r)}><Download size={14}/> PDF</button>}{r.pdf_object_path&&r.match_status!=="matched"&&<button className="secondary" disabled={parsingId===r.id} onClick={()=>parsePdf(r)}>{parsingId===r.id?"Reading…":"Read PDF"}</button>}{r.match_status!=="matched"&&<button className="primary" onClick={()=>{setSelected(r);setJobId("");}}>Match to job</button>}</div></article>)}{!visible.length&&<div className="empty">No close-outs found.</div>}</div>{selected&&<div className="modal-backdrop"><div className="modal mini-modal"><div className="modal-header"><div><h2>Match FastField close-out</h2><p>Job {selected.raw_job_number||"not supplied"} · WO {selected.work_order_number||"not supplied"}</p></div><button className="icon" onClick={()=>setSelected(null)}><X size={18}/></button></div><label>AIM job<select value={jobId} onChange={e=>setJobId(e.target.value)}><option value="">Select job</option>{jobs.filter(j=>!j.isAdHoc&&!j.isTravelComment).sort((a,b)=>String(a.jobNumber||"").localeCompare(String(b.jobNumber||""))).map(j=><option key={j.id} value={j.id}>{j.jobNumber||"No job no."} · {j.workOrderNumber||"No WO"} · {j.title}</option>)}</select></label><div className="modal-actions"><button className="secondary" onClick={()=>setSelected(null)}>Cancel</button><button className="primary" disabled={!jobId||busy} onClick={match}>{busy?"Matching…":"Attach to job"}</button></div></div></div>}</section></main>;
}
...
function SideNav({ unreadMessages, lowStockCount, isAdmin, hasStoresPermission, onCalendar, onShare, onMessages, onPeople, onReports, onCloseouts, onJobPacks, onTools, onInventory, onMachinery, onSettings }) {
  return (
    <nav className="side-nav" aria-label="Main actions">
      ...
      {isAdmin && <button title="FastField close-outs" onClick={onCloseouts}><ClipboardList size={22}/></button>}
      {isAdmin && <button title="Tradify job-pack imports" onClick={onJobPacks}><FolderInput size={22}/></button>}
                    ...
                    {job.isAdHoc ? (
                      ...
                      <>
                        ...
                        <div className="employee-card-tabs">
                          ...
                          <details><summary>Materials list</summary><TradeJobMaterials job={job} items={inventoryItems} locations={inventoryLocations} movements={inventoryMovements}/></details>
...
async function saveInventoryTransferToSupabase(m){const base={itemId:m.itemId,quantity:Number(m.quantity),unitCost:Number(m.unitCost)||0,reference:m.reference||null,notes:m.notes||null,createdBy:m.createdBy};const transferRef=m.reference||`Transfer ${new Date().toISOString()}`;const transferGroupId=crypto.randomUUID();const rows=[{...base,locationId:m.locationId,movementType:"transfer_out",reference:transferRef,notes:m.notes||`Transferred to ${m.toLocationId}`},{...base,locationId:m.toLocationId,movementType:"transfer_in",reference:transferRef,notes:m.notes||`Transferred from ${m.locationId}`}];const payload=rows.map(x=>({item_id:x.itemId,location_id:x.locationId,job_id:null,movement_type:x.movementType,quantity:x.quantity,unit_cost:x.unitCost,total_cost:x.quantity*x.unitCost,reference:x.reference,notes:x.notes,created_by:isUuid(x.createdBy)?x.createdBy:null,transfer_group_id:transferGroupId}));const {error}=await supabase.from("inventory_movements").insert(payload);if(error)throw error;}
