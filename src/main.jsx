...
Reply: ${messageText}` };
  ...
  async function revokeWorkerAccess(worker) {
    if (!supabase || !worker?.id) return;
    const { data, error } = await supabase.functions.invoke("revoke-worker", { body: { worker_id: worker.id } });
    if (error) throw error;
  ...
  return (
    <div className={activeView === "admin" ? "app admin-mode" : currentUser ? "app nav-mode employee-mode" : "app"}>
      ...
      {settingsOpen && currentUser && <SettingsModal currentUser={currentUser} currentRole={currentRole} isAdminUser={isAdminUser} activeView={activeView} isInstalledPwa={isInstalledPwa} canPromptInstall={Boolean(installPrompt)} onInstall={installJobsched} onOpenCloseouts={()=>{setSettingsOpen(false);setToolsOpen(false);setInventoryOpen(false);setMachineryOpen(false);setReportsOpen(false);setJobPacksOpen(false);setCloseoutsOpen(true);}} onOpenJobPacks={()=>{setSettingsOpen(false);setToolsOpen(false);setInventoryOpen(false);setMachineryOpen(false);setReportsOpen(false);setCloseoutsOpen(false);setJobPacksOpen(true);}} onClose={()=>setSettingsOpen(false)} onSetView={(nextView)=>{setView(nextView); setSettingsOpen(false);}} onChangePassword={()=>{setPasswordSetupMode("manual"); setSettingsOpen(false);}} onSignOut={async()=>{setSettingsOpen(false); await signOut();}} />}
...
function SettingsModal({ currentUser, currentRole, isAdminUser, activeView, isInstalledPwa, canPromptInstall, onInstall, onOpenCloseouts, onOpenJobPacks, onClose, onSetView, onChangePassword, onSignOut }) {
  return (
    <div className="modal-backdrop">
