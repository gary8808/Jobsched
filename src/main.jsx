...
import {
  ...
  AlertCircle, CalendarX, Plane, Wrench, RotateCcw, UserMinus
} from "lucide-react";
...
const STORAGE_KEY = "jobsched-v16-safety-action-needed";
...
function App() {
    ...
    async function checkSupabase() {
      ...
      try {
        const { data: workers, error } = await supabase
          .from("workers")
          .select("id,name,trade,base_site,email")
        ...
        setSupabaseCheck({
          status: "connected",
          workers: workers || [],
          message: `Authenticated as ${session.user?.email || "user"}. Workers found: ${(workers || []).length}`
        });
      } catch (error) {
        if (!alive) return;
        setSupabaseCheck({
  ...
  function scheduleJob(jobId, workerId, date, dragContext = {}) {
    updateJobs(job => {
      ...
      if (wasConfirmed && (!jobOccursForWorkerOnDate(job, workerId, date) || isMovingFromCalendar)) {
      ...
      if (!hasPrimaryBooking(job)) {
        updated = { ...job, assignedTo: [workerId], startDate: date, endDate: date, category: "Scheduled", clientAccepted: keepConfirmed };
      ...
      updated = logJob(updated, "Scheduled", isMovingFromCalendar ? `Moved booking to ${getWorkerName(data.teamMembers, workerId)} on ${date}.` : `Booked to ${getWorkerName(data.teamMembers, workerId)} on ${date}.`);
  ...
  function sendDemoMessage(job, customText = "") {
    const messageText = customText || buildScheduleMessage(job);
    const message = { id: createId(), jobId: job.id, direction: "out", channel: "sms", from: CURRENT_USER, text: messageText, date: new Date().toISOString(), unread: false };
    const reply = { id: createId(), jobId: job.id, direction: "in", channel: "sms", from: job.clientContact || "Client", text: "Demo reply: Confirmed, thank you.", date: new Date(Date.now()+1000).toISOString(), unread: true };
  ...
  function confirmJobComplete(jobId) {
    updateJobs(job => {
      if (job.id !== jobId) return job;
      return logJob({ ...job, category: "Completed", completedConfirmed: true }, "Completed", "Supervisor confirmed job complete from calendar.");
    });
  }
  ...
  const topButtons = (
    <div className="actions top-menu">
      {currentUser && <span className="current-user-pill">{currentUser.email}</span>}
      {currentUser && <button className="secondary" onClick={signOut}>Sign out</button>}
  ...
  return (
    <div className={view === "admin" ? "app admin-mode" : "app"}>
      ...
      {view === "admin" && <SupabaseTestPanel result={supabaseCheck} user={currentUser} />}

      {!currentUser ? (
        <AuthPanel onSignIn={signIn} loading={authLoading} />
      ) : view === "admin" ? (
        <>
          ...
          <main className="workspace">
            ...
            <section className="calendar-area">
              ...
              {selectedBucketJobId && <div className="selected-job-banner"><strong>Selected job:</strong> {data.jobs.find(j=>j.id===selectedBucketJobId)?.title || "Job"}<span>Tap a calendar cell to book it, or drag it into the calendar.</span><button className="ghost" onClick={()=>setSelectedBucketJobId(null)}>Clear</button></div>}<CalendarGrid days={days} workers={visibleWorkers} jobs={scheduledJobs} leaveRecords={data.leaveRecords} messages={data.messages} selectedJobId={selectedBucketJobId} onCellSelectBooking={bookSelectedJob} onDropJob={scheduleJob} onDragStart={handleDragStart} onAddItem={setCalendarPopup} onEditJob={setEditingJob} onDeleteJob={cancelOrDeleteJob} onToggle={toggleJobCheckbox} onConfirmComplete={confirmJobComplete}/>
            </section>
          </main>
          )}
        </>
      ...
      {editingJob && <JobModal job={editingJob} teamMembers={data.teamMembers} onClose={()=>setEditingJob(null)} onSave={saveJob} onSendMessage={sendDemoMessage} />}
...
function SupabaseTestPanel({ result, user }) {
      ...
      {connected && result.workers?.length > 0 && (
        <div className="supabase-worker-list">
          {result.workers.map(worker => (
            <span key={worker.id}>{worker.name || "Unnamed"}{worker.trade ? ` · ${worker.trade}` : ""}{worker.base_site ? ` · ${worker.base_site}` : ""}</span>
          ))}
        </div>
      )}
    </section>
...
function AuthPanel({ onSignIn, loading }) {
  ...
  return (
    <main className="auth-page">
      <form className="auth-card" onSubmit={submit}>
        ...
        <p className="muted">Once sign-in works, the temporary anon workers policy can be removed.</p>
      </form>
    </main>
  );
}
...
function CalendarGrid({ days, workers, jobs, leaveRecords, messages, selectedJobId, onCellSelectBooking, onDropJob, onDragStart, onAddItem, onEditJob, onDeleteJob, onToggle, onConfirmComplete }) {
  return <div className="calendar-wrap"><div className="calendar-grid" style={{"--day-count": days.length}}><div className="corner-cell">Workers</div>{days.map(day=><div key={getIsoDate(day)} className={`day-header ${isToday(day)?"today":""}`}><strong>{formatDayName(day)}</strong><span>{formatDateHeader(day)}</span></div>)}{workers.map(worker=><React.Fragment key={worker.id}><div className="worker-cell"><button className="worker-profile-button" type="button"><strong>{worker.name || "Unnamed worker"}</strong><span>{worker.trade||"No trade"} · {worker.baseSite||"No site"}</span><em>{worker.sapNumber||"No SAP"}</em></button></div>{days.map(day=>{const iso=getIsoDate(day); const availability=getWorkerAvailability(worker, iso, leaveRecords); const dayLeaves=leaveRecords.filter(l=>l.workerId===worker.id && isDateWithinRange(iso,l.startDate,l.endDate)); const cellJobs=jobs.filter(j=>jobOccursForWorkerOnDate(j, worker.id, iso)).sort(sortScheduleItems); return <div key={`${worker.id}-${iso}`} className={`calendar-cell ${isToday(day)?"today-cell":""} ${availability.status==="Onsite"?"onsite-cell":"rnr-cell"}`} onDragOver={e=>{e.preventDefault(); e.dataTransfer.dropEffect="move";}} onClick={(e)=>{ if (selectedJobId && e.target === e.currentTarget) onCellSelectBooking(worker.id, iso); }} onDrop={e=>{e.preventDefault(); const context=readDragContext(e); const id=context.jobId || e.dataTransfer.getData("text/plain"); if(id) onDropJob(id, worker.id, iso, context);}}><div className={`roster-badge ${availability.status==="Onsite"?"onsite":"rnr"}`}>{availability.label}</div><button className="add-cell-job" onClick={()=>onAddItem({workerId:worker.id,workerName:worker.name,date:iso})}><Plus size={14}/> Item</button><div className="cell-jobs">{dayLeaves.map(l=><LeaveCard key={l.id} leave={l} />)}{cellJobs.map(job=><CalendarJob key={`${job.id}-${worker.id}-${iso}`} job={job} workerId={worker.id} isStart={isJobOccurrenceStart(job, worker.id, iso)} hasUnreadMessage={messages.some(m=>m.jobId===job.id && m.unread)} onDragStart={e=>onDragStart(e,job.id,worker.id,iso)} onEdit={()=>onEditJob(job)} onDelete={()=>onDeleteJob(job)} onToggleAppointmentSent={()=>onToggle(job.id,"appointmentSent")} onToggleClientAccepted={()=>onToggle(job.id,"clientAccepted")} onConfirmComplete={()=>onConfirmComplete(job.id)}/>)}</div></div>})}</React.Fragment>)}</div></div>;
}

function CalendarJob({ job, workerId, isStart, hasUnreadMessage, onDragStart, onEdit, onDelete, onToggleAppointmentSent, onToggleClientAccepted, onConfirmComplete }) {
  ...
  else if (["Awaiting parts"].includes(job.category) || !["Ready", "Not required"].includes(job.materialsStatus || "Not checked")) tabClass = "materials";
  else if (needsAnotherTrade) tabClass = "followup";
  else if (job.clientAccepted) tabClass = "confirmed";
  return (
    <article className={`calendar-job ${job.category === "Completed" || job.completedConfirmed ? "supervisor-complete" : ""} ${completedByTrade && job.category !== "Completed" ? "trade-complete" : ""} ${job.isAdHoc?"ad-hoc":""} ${job.isTravelComment?"travel-comment":""} ${isStart?"range-start":"range-middle"}`} draggable onDragStart={onDragStart}>
      <div className="range-body">
        {hasUnreadMessage && <span className="job-envelope"><Mail size={13}/></span>}
        {isStart ? (
          <>
            <div className="card-top">
              <span className="calendar-ref-row"><span className="wo">{job.isTravelComment ? "Travel" : job.isAdHoc ? "Ad hoc" : job.quoteNumber || job.jobNumber || "No QUO"}</span>{!job.isAdHoc && !job.isTravelComment && getSafetyPermits(job).length > 0 && <span className={`safety-hardhat ${allSafetyOrganised(job) ? "safe" : "unsafe"}`} title={allSafetyOrganised(job) ? "Safety/permits organised" : "Safety/permits still required"}>⛑</span>}</span>
            ...
            <h3>{job.title}</h3>
            {!job.isAdHoc && !job.isTravelComment && job.client && <div className="meta compact-meta"><span>{job.client}</span></div>}
            ...
            {completedByTrade && job.category !== "Completed" && !job.isAdHoc && !job.isTravelComment && <button className="confirm-complete-button" onClick={onConfirmComplete}>Confirm complete</button>}
          </>
...
function JobCard({ job, workerNames, selected, onSelect, onDragStart, onEdit, onDelete }) {
  return (
    <article className={`card compact bucket-card-minimal ${selected ? "selected" : ""}`} draggable onClick={onSelect} onDragStart={onDragStart} onDoubleClick={onEdit}>
      <div className="card-top">
...
function JobModal({ job, teamMembers, onClose, onSave, onSendMessage }) {
  const [form, setForm] = useState(normaliseJob(job));
  const [activeTab, setActiveTab] = useState("details");
  const [noteInput, setNoteInput] = useState("");
  const [materialInput, setMaterialInput] = useState("");
  const [messageText, setMessageText] = useState(buildScheduleMessage(form));
  ...
  function updateSafetyPermit(id, organised){
    setForm(cur=>({...cur,safetyPermits:getSafetyPermits(cur).map(p=>p.id===id?{...p,organised}:p)}));
  }
  ...
  {activeTab==="client"&&<section className="job-tab-panel"><div className="two-col"><label>Client contact<input value={form.clientContact||""} onChange={e=>update("clientContact",e.target.value)}/></label><label>Client phone<input value={form.clientPhone||""} onChange={e=>update("clientPhone",e.target.value)}/></label></div><div className="appointment-panel stacked"><label className="check-option plain"><input type="checkbox" checked={!!form.appointmentSent} onChange={e=>update("appointmentSent",e.target.checked)}/>Appointment SMS sent</label><label className="check-option plain"><input type="checkbox" checked={!!form.clientAccepted} onChange={e=>update("clientAccepted",e.target.checked)}/>Client accepted appointment</label><div className="message-template-actions"><button type="button" className="secondary" onClick={()=>setMessageText(buildScheduleMessage(form))}>Scheduling message</button><button type="button" className="secondary" onClick={()=>setMessageText(buildRescheduleMessage(form))}>Reschedule message</button></div><label className="full-width-label">Demo message text<textarea rows="5" value={messageText} onChange={e=>setMessageText(e.target.value)}/></label><button type="button" className="secondary" onClick={()=>{if(onSendMessage(form, messageText)) update("appointmentSent",true)}}><MessageSquare size={16}/> Send demo message</button><a className="secondary" href={form.clientPhone?`tel:${form.clientPhone}`:undefined} onClick={(e)=>{if(!form.clientPhone){e.preventDefault(); alert("Enter a client phone number first.")}}}><Phone size={16}/> Call client</a></div></section>}
...
function CalendarItemModal({ context, onClose, onSave }) {
  ...
  async function submit(e) {
    ...
    const attachments = mode === "travel" && travelPdf ? [await fileToAttachment(travelPdf, "Accommodation confirmation PDF")] : [];
    onSave({ type: mode, workerId: context.workerId, text, startDate, endDate, attachments });
  }
  ...
  return (
    <section className="safety-permit-panel">
      ...
      {permits.length > 0 && <div className="safety-permit-list">
        {permits.map(permit => (
          <div key={permit.id} className={permit.organised ? "safety-permit-row organised" : "safety-permit-row outstanding"}>
            <span>{permit.name}</span>
            <label className="check-option plain"><input type="checkbox" checked={!!permit.organised} onChange={e => onToggle(permit.id, e.target.checked)} />Organised</label>
...
function PeopleModal({ workers, onClose, onSave }) {
  const [items, setItems] = useState(workers.map(w => ({ inactive: false, accessRevoked: false, ...w })));
  ...
  function add() {
    const worker = { id: "worker-" + createId(), name: "", trade: "", baseSite: "", phone: "", email: "", birthday: "", sapNumber: "", rosterPattern: "NONE", rosterStartDate: "", inactive: false, accessRevoked: false };
  ...
  function deactivate(worker) {
    ...
    update(worker.id, "inactive", true);
    update(worker.id, "accessRevoked", revoke);
    if (revoke) alert("Demo only: access would be revoked in the backend user account system.");
  }

  return <div className="modal-backdrop"><div className="modal settings-modal"><div className="modal-header"><h2>People</h2><button className="icon" onClick={onClose}><X size={18}/></button></div><div className="choice-grid two-choice"><button className="choice-card" onClick={add}><Users/><strong>Add employee</strong><span>Create a new employee profile.</span></button><button className="choice-card active" onClick={()=>setMode("edit")}><Pencil/><strong>Edit employee</strong><span>Select an employee from the list and edit their details.</span></button></div><div className="people-layout"><aside className="people-list"><label>Name filter<input value={filter} onChange={e=>setFilter(e.target.value)} placeholder="Search employee..."/></label>{filtered.map(w=><button key={w.id} className={selected?.id===w.id?"active":""} onClick={()=>setSelectedId(w.id)}><strong>{w.name||"Unnamed employee"}</strong><span>{w.trade||"No trade"}{w.inactive ? " · inactive" : ""}</span></button>)}</aside>{selected&&<section className="worker-editor"><div className="two-col"><label>Name<input value={selected.name} onChange={e=>update(selected.id,"name",e.target.value)}/></label><label>Trade<select value={selected.trade||""} onChange={e=>update(selected.id,"trade",e.target.value)}><option value="">Select</option>{TRADES.map(t=><option key={t}>{t}</option>)}</select></label></div><div className="two-col"><label>Phone<input value={selected.phone||""} onChange={e=>update(selected.id,"phone",e.target.value)}/></label><label>Email<input value={selected.email||""} onChange={e=>update(selected.id,"email",e.target.value)}/></label></div><div className="two-col"><label>Birthday<input type="date" value={selected.birthday||""} onChange={e=>update(selected.id,"birthday",e.target.value)}/></label><label>SAP number<input value={selected.sapNumber||""} onChange={e=>update(selected.id,"sapNumber",e.target.value)}/></label></div><div className="two-col"><label>Base site<select value={selected.baseSite||""} onChange={e=>update(selected.id,"baseSite",e.target.value)}><option value="">Select</option>{BASE_SITES.map(s=><option key={s}>{s}</option>)}</select></label><label>Roster<select value={selected.rosterPattern||"NONE"} onChange={e=>update(selected.id,"rosterPattern",e.target.value)}>{ROSTER_PATTERNS.map(r=><option key={r.id} value={r.id}>{r.label}</option>)}</select></label></div>{selected.rosterPattern==="CUSTOM" ? <CustomRosterEditor worker={selected} update={(field,value)=>update(selected.id,field,value)} /> : <label>Roster start date<input type="date" value={selected.rosterStartDate||""} onChange={e=>update(selected.id,"rosterStartDate",e.target.value)}/></label>}<label className="check-option plain"><input type="checkbox" checked={!!selected.inactive} onChange={e=>e.target.checked ? deactivate(selected) : update(selected.id,"inactive",false)}/>Deactivate employee</label>{selected.accessRevoked&&<p className="muted">Access marked for revocation.</p>}</section>}</div><div className="modal-actions"><button className="secondary" onClick={onClose}>Cancel</button><button className="primary" onClick={()=>onSave(items)}>Save people</button></div></div></div>;
}
...
function TravelEmployeeCard({ job }) {
  const pdf = (job.attachments || []).find(a => a.dataUrl || a.url);
  return (
    <div className="travel-employee-panel">
      ...
      {pdf ? <a className="secondary travel-download" href={pdf.dataUrl || pdf.url} download={pdf.name || "accommodation-confirmation.pdf"}><Paperclip size={16}/> Download accommodation PDF</a> : <p className="muted">No accommodation PDF attached.</p>}
    </div>
  );
}
...
function normaliseSafetyPermits(value){
  ...
  return value.map(item => {
    if(typeof item === "string") return { id: createId(), name: item, organised: false };
    return { id: item.id || createId(), name: item.name || item.label || "", organised: Boolean(item.organised) };
