...
function App() {
  ...
  function scheduleJob(jobId, workerId, date) {
    updateJobs(job => {
      ...
      if (wasConfirmed && !jobOccursForWorkerOnDate(job, workerId, date)) {
      ...
      updated = logJob(updated, "Scheduled", `Booked to ${getWorkerName(data.teamMembers, workerId)} on ${date}.`);
        ...
        <>
          ...
          <main className="workspace">
            ...
            <section className="calendar-area">
              ...
              <CalendarGrid days={days} workers={visibleWorkers} jobs={scheduledJobs} leaveRecords={data.leaveRecords} messages={data.messages} onDropJob={scheduleJob} onDragStart={handleDragStart} onAddItem={setCalendarPopup} onEditJob={setEditingJob} onDeleteJob={cancelOrDeleteJob} onToggle={toggleJobCheckbox} onConfirmComplete={confirmJobComplete}/>
            </section>
          </main>
          )}
        </>
...
function JobCard({ job, workerNames, onDragStart, onEdit, onDelete }) {
  return (
    <article className="card compact bucket-card-minimal" draggable onDragStart={onDragStart} onDoubleClick={onEdit}>
      <div className="card-top">
...
function JobModal({ job, teamMembers, onClose, onSave, onSendMessage }) {
  ...
  function updateScheduleBlock(id, field, value){ setForm(cur=>({...cur,scheduleBlocks:(cur.scheduleBlocks||[]).map(b=>b.id===id?{...b,[field]:value}:b)})); }
