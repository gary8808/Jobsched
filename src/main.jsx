...
function App() {
  ...
  const topButtons = (
    <div className="actions top-menu">
      ...
      {currentUser && <button className="secondary" onClick={() => setPasswordSetupMode("manual")}>Set/change password</button>}
  ...
  return (
    <div className={view === "admin" ? "app admin-mode" : "app"}>
      ...
      {peopleOpen && <PeopleModal workers={data.teamMembers} usingSupabase={Boolean(session && supabase)} saving={workersLoading} onClose={()=>setPeopleOpen(false)} onSave={async (workers)=>{
        try {
          const freshWorkers = await saveWorkers(workers);
          const inviteRequests = workers.filter(w => w.sendInvite && w.email && !w.inactive);
          const sent = [];
          for (const request of inviteRequests) {
            const savedWorker = (freshWorkers || workers).find(w =>
      ...
      <div className="modal settings-modal">
        ...
        <div className="people-layout">
          <aside className="people-list">
            ...
            {filtered.map(w=><button key={w.id} type="button" className={selected?.id===w.id?"active":""} onClick={()=>setSelectedId(w.id)}><strong>{w.name||"Unnamed employee"}</strong><span>{w.trade||"No trade"}{w.inactive ? " · inactive" : ""}{w.appRole ? ` · ${w.appRole}` : ""}</span></button>)}
