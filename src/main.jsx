...
// AIM CG v43 - trade notes, materials workflow and tool register
...
const STORAGE_KEY = "aim-cg-v43-tools";
...
Reply: ${messageText}` };
  ...
  return (
    <div className={activeView === "admin" ? "app admin-mode" : currentUser ? "app nav-mode employee-mode" : "app"}>
      ...
      {toolsOpen && currentUser && <ToolRegisterModal tools={tools} history={toolHistory} workers={data.teamMembers} currentWorkerId={employeeId} isAdmin={isAdminUser} onClose={()=>setToolsOpen(false)} onRefresh={async()=>{setTools(await fetchToolsFromSupabase());setToolHistory(await fetchToolHistoryFromSupabase());}} />}
...
function ToolRegisterModal({ tools, history, workers, currentWorkerId, isAdmin, onClose, onRefresh }) {
  const [query, setQuery] = useState("");
  ...
  return <div className="modal-backdrop"><div className="modal tool-register-modal"><div className="modal-header"><div><h2>Tool Register</h2><p>{isAdmin ? "Manage tool allocation, status and history." : "View your tools, sign out available tools and report faults."}</p></div><button className="icon" onClick={onClose}><X size={18}/></button></div>
    <div className="tool-register-actions"><div className="search"><Search size={16}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Filter by description or tool ID..."/></div>{isAdmin&&<button className="primary" onClick={()=>setEditing({id:createId(),description:"",toolId:"",serialNumber:"",brandModel:"",status:"available",assignedWorkerId:"",notes:"",active:true})}><Plus size={16}/> Add tool</button>}</div>
...
async function fetchToolsFromSupabase() {
  ...
  return (data||[]).map(row=>({id:row.id,description:row.description||"",toolId:row.tool_id||"",serialNumber:row.serial_number||"",brandModel:row.brand_model||"",status:row.status||"available",assignedWorkerId:row.assigned_worker_id||"",signedOutAt:row.signed_out_at||"",notes:row.notes||"",active:row.active!==false}));
}
...
async function saveToolToSupabase(tool) {
  if (!supabase) throw new Error("Supabase is not configured.");
  const row={id:tool.id,description:tool.description,tool_id:tool.toolId,serial_number:tool.serialNumber||null,brand_model:tool.brandModel||null,status:tool.assignedWorkerId && tool.status==="available" ? "signed_out" : (tool.status||"available"),assigned_worker_id:tool.assignedWorkerId||null,signed_out_at:tool.assignedWorkerId?(tool.signedOutAt||new Date().toISOString()):null,notes:tool.notes||null,active:tool.active!==false,updated_at:new Date().toISOString()};
  const {error}=await supabase.from("tools").upsert(row,{onConflict:"id"}); if(error)throw error;
}
