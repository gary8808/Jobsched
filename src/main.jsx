        ...
        {isStart ? (
          <>
            <div className="card-top">
              <span className="calendar-ref-row"><span className="wo">{job.isTravelComment ? "Travel" : job.isAdHoc ? "Ad hoc" : job.quoteNumber || job.jobNumber || "No QUO"}</span>{getSafetyPermits(job).length > 0 && <span className={`safety-hardhat ${allSafetyOrganised(job) ? "safe" : "unsafe"}`} title={allSafetyOrganised(job) ? "Safety/permits organised" : "Safety/permits still required"}>⛑</span>}</span>
    ...
    <main className="employee-view app-like-view">
      ...
      <section className="employee-list">
        {displayDays.map(day => {
          ...
          const dayJobs = visible.filter(j => jobOccursForWorkerOnDate(j, worker?.id || "", iso));
          return (
