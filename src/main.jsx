...
// AIM CG v53 - warehouse role, read-only warehouse calendar, stability cleanup
        ...
        <>
          ...
          <main className="workspace">
            <aside className={`bucket-panel ${bucketsCollapsed ? "collapsed" : ""}`}>
              ...
              <div className="selected-bucket-list">{filteredJobs.map(job=><JobCard key={job.id} job={job} workerNames={getAssignedWorkerNames(data.teamMembers, job.assignedTo)} selected={selectedBucketJobId===job.id} onSelect={()=>setSelectedBucketJobId(job.id)} onDragStart={e=>{if(isAdminUser)handleDragStart(e,job.id)}} onEdit={()=>{if(isAdminUser)setEditingJob(job)}} onDelete={()=>{if(isAdminUser)cancelOrDeleteJob(job)}} readOnly={isWarehouseUser}/>) }{filteredJobs.length===0 && <div className="empty small">No jobs in this bucket</div>}</div>
              </div>
            </aside>
...
function CalendarJob({ job, workerId, date, isStart, selected, hasAnyMessage, hasUnreadMessage, onSelect, onOpenMessages, onDragStart, onEdit, onDelete, onToggleAppointmentSent, onToggleClientAccepted, onConfirmComplete, readOnly = false }) {
  ...
  return (
    <article className={`calendar-job ${selected ? "selected-booking" : ""} ${defectJob ? "defect-callback" : ""} ${job.category === "Completed" || job.completedConfirmed ? "supervisor-complete" : ""} ${completedByTrade && job.category !== "Completed" ? "trade-complete" : ""} ${job.isAdHoc?"ad-hoc":""} ${job.isTravelComment?"travel-comment":""} ${isStart?"range-start":"range-middle"}`} draggable={!readOnly} onDragStart={readOnly?undefined:onDragStart} onClick={readOnly?undefined:onSelect}>
