...
Reply: ${messageText}` };
  ...
  return (
    <div className={activeView === "admin" ? "app admin-mode" : currentUser ? "app nav-mode employee-mode" : "app"}>
      {currentUser && <SideNav unreadMessages={unreadMessages} lowStockCount={lowStockCount} isAdmin={isAdminUser} hasStoresPermission={hasStoresPermission} onCalendar={() => { setToolsOpen(false); setInventoryOpen(false); setMachineryOpen(false); setReportsOpen(false); setCloseoutsOpen(false); if (isAdminUser) setAdminTab("schedule"); }} onShare={() => setShareHubOpen(true)} onMessages={() => setMessagesOpen(true)} onPeople={() => setPeopleOpen(true)} onReports={() => {setToolsOpen(false);setInventoryOpen(false);setMachineryOpen(false);setCloseoutsOpen(false);setReportsOpen(true);}} onCloseouts={() => {setToolsOpen(false);setInventoryOpen(false);setMachineryOpen(false);setReportsOpen(false);setCloseoutsOpen(true);}} onTools={() => {setReportsOpen(false);setCloseoutsOpen(false);setInventoryOpen(false);setMachineryOpen(false);setToolsOpen(true);}} onInventory={() => {setReportsOpen(false);setCloseoutsOpen(false);setToolsOpen(false);setMachineryOpen(false);setInventoryOpen(true);}} onMachinery={() => {setReportsOpen(false);setCloseoutsOpen(false);setToolsOpen(false);setInventoryOpen(false);setMachineryOpen(true);}} onSettings={() => setSettingsOpen(true)} />}
...
function FastFieldCloseoutsPage({jobs,onClose}){
  const [rows,setRows]=useState([]);const [query,setQuery]=useState("");const [selected,setSelected]=useState(null);const [jobId,setJobId]=useState("");const [busy,setBusy]=useState(false);
  async function load(){if(!supabase)return;const {data,error}=await supabase.from("fastfield_submissions").select("*").order("received_at",{ascending:false});if(error){alert(`Could not load FastField submissions: ${error.message}`);return;}setRows(data||[]);}
  useEffect(()=>{load();},[]);
  async function match(){if(!selected||!jobId)return;setBusy(true);const {error}=await supabase.rpc("aimcg_match_fastfield_submission",{p_submission_id:selected.id,p_job_id:jobId});setBusy(false);if(error){alert(error.message);return;}setSelected(null);setJobId("");await load();}
  async function openPdf(row){if(!row.pdf_bucket||!row.pdf_object_path)return;const {data,error}=await supabase.storage.from(row.pdf_bucket).createSignedUrl(row.pdf_object_path,120);if(error){alert(error.message);return;}window.open(data.signedUrl,"_blank","noopener,noreferrer");}
  const q=query.trim().toLowerCase();const visible=rows.filter(r=>!q||[r.raw_job_number,r.work_order_number,r.reference_text,r.submitted_by_email,r.match_status].join(" ").toLowerCase().includes(q));
  return <main className="reports-page"><section className="reports-page-shell"><div className="inventory-header"><div><h1><ClipboardList size={28}/> FastField close-outs</h1><p>Matched reports attach automatically. Unrelated or future jobs remain safely in the unmatched queue.</p></div><div className="report-actions"><button className="secondary" onClick={load}><RotateCcw size={15}/> Refresh</button><button className="secondary" onClick={onClose}><ChevronLeft size={17}/> Back</button></div></div><div className="search closeout-search"><Search size={16}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search job number, work order, reference or submitter…"/></div><div className="closeout-summary"><span>{rows.filter(r=>r.match_status==="unmatched").length} unmatched</span><span>{rows.filter(r=>r.match_status==="matched").length} matched</span><span>{rows.filter(r=>r.further_work_identified).length} further-work flags</span></div><div className="closeout-list">{visible.map(r=><article className={`closeout-row ${r.further_work_identified?"warning":""}`} key={r.id}><div><strong>{r.raw_job_number?`Job ${r.raw_job_number}`:"No job number"} · WO {r.work_order_number||"not supplied"}</strong><span>{r.reference_text||r.form_name||"FastField close-out"}</span><em>{r.submitted_by_email||r.submitted_by_name||"Unknown submitter"} · {formatDateTime(r.submitted_at||r.received_at)}</em><small>Status: {r.match_status} / {r.processing_status}{r.further_work_identified?" · Further work identified":""}</small></div><div className="closeout-actions">{r.pdf_object_path&&<button className="secondary" onClick={()=>openPdf(r)}><Download size={14}/> PDF</button>}{r.match_status!=="matched"&&<button className="primary" onClick={()=>{setSelected(r);setJobId("");}}>Match to job</button>}</div></article>)}{!visible.length&&<div className="empty">No close-outs found.</div>}</div>{selected&&<div className="modal-backdrop"><div className="modal mini-modal"><div className="modal-header"><div><h2>Match FastField close-out</h2><p>Job {selected.raw_job_number||"not supplied"} · WO {selected.work_order_number||"not supplied"}</p></div><button className="icon" onClick={()=>setSelected(null)}><X size={18}/></button></div><label>AIM job<select value={jobId} onChange={e=>setJobId(e.target.value)}><option value="">Select job</option>{jobs.filter(j=>!j.isAdHoc&&!j.isTravelComment).sort((a,b)=>String(a.jobNumber||"").localeCompare(String(b.jobNumber||""))).map(j=><option key={j.id} value={j.id}>{j.jobNumber||"No job no."} · {j.workOrderNumber||"No WO"} · {j.title}</option>)}</select></label><div className="modal-actions"><button className="secondary" onClick={()=>setSelected(null)}>Cancel</button><button className="primary" disabled={!jobId||busy} onClick={match}>{busy?"Matching…":"Attach to job"}</button></div></div></div>}</section></main>;
}

function SideNav({ unreadMessages, lowStockCount, isAdmin, hasStoresPermission, onCalendar, onShare, onMessages, onPeople, onReports, onCloseouts, onTools, onInventory, onMachinery, onSettings }) {
  return (
