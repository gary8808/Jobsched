...
import {
  ...
  AlertCircle, CalendarX, Plane, Wrench, RotateCcw, UserMinus, Settings, ZoomIn, ZoomOut
} from "lucide-react";
...
function App() {
  ...
  const calendarDayMin = calendarDayCount >= 14 ? "135px" : calendarDayCount >= 10 ? "165px" : calendarDayCount <= 5 ? "280px" : "230px";
  ...
  return (
    <div className={activeView === "admin" ? "app admin-mode" : currentUser ? "app nav-mode" : "app"}>
      ...
      {activeView === "admin" && <SupabaseTestPanel result={supabaseCheck} user={currentUser} />}
...
function CalendarJob({ job, workerId, isStart, hasUnreadMessage, onDragStart, onEdit, onDelete, onToggleAppointmentSent, onToggleClientAccepted, onConfirmComplete }) {
  ...
  if (defectJob) tabClass = "defect";
  else if (job.category === "Completed" || job.completedConfirmed) tabClass = "completed";
  ...
  return (
    <article className={`calendar-job ${defectJob ? "defect-callback" : ""} ${job.category === "Completed" || job.completedConfirmed ? "supervisor-complete" : ""} ${completedByTrade && job.category !== "Completed" && !defectJob ? "trade-complete" : ""} ${job.isAdHoc?"ad-hoc":""} ${job.isTravelComment?"travel-comment":""} ${isStart?"range-start":"range-middle"}`} draggable onDragStart={onDragStart}>
...
function EmployeeView({ workerId, setWorkerId, workers, days, jobs, leaveRecords, onAddNote, onStatus, onAddAttachment, onCompletion, canSwitchWorker = false }) {
