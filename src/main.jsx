...
function App() {
  ...
  const filteredJobs = useMemo(() => {
    ...
    return bucketJobs.filter(job => {
      ...
      const matchesTag = tagFilter === "All" || (job.tags || []).some(tag => tag.toLowerCase() === tagFilter.toLowerCase());
      const matchesQuery = !q || [job.title, job.client, job.jobNumber, job.quoteNumber, job.workOrderNumber, job.poNumber, job.address, job.clientContact, job.notes, ...(job.tags||[])].join(" ").toLowerCase().includes(q);
...
function jobSearchText(job){return [job.jobNumber,job.workOrderNumber,job.quoteNumber,job.poNumber,job.title,job.client,job.address,job.site,...(job.tags||[])].join(" ").toLowerCase();}
