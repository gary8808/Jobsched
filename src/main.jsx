...
const STORAGE_KEY = "jobsched-v32-employee-updates-supabase";
  ...
  function sendDemoMessage(job, customText = "") {
    ...
    if (changedJob && session && supabase) {
      ...
      insertJobHistoryToSupabase({
        jobId: job.id,
        action: "Message sent",
        details: "Demo SMS generated from Jobsched.",
  ...
  function addEmployeeAttachments(jobId, workerId, files) {
    ...
    if (session && supabase) {
      insertJobNoteToSupabase({
        jobId,
        workerId,
        noteText: `${workerName} added ${fileList.length} photo/file attachment(s). File metadata is saved to the job; Supabase Storage upload comes in the attachments phase.`,
        noteType: "photo_metadata",
        createdBy: currentUser?.id || null
  ...
  function saveEmployeeCompletion(jobId, workerId, completion) {
    ...
    if (changedJob && session && supabase) {
      ...
      insertJobNoteToSupabase({
        ...
        createdBy: currentUser?.id || null
      }).catch(err => console.error("Could not save completion note", err));
      insertJobHistoryToSupabase({
        jobId,
        ...
        details: completion.requiresAnotherTrade
          ...
          : `${workerName} saved a completion update.`,
        createdBy: currentUser?.id || null
      }).catch(err => console.error("Could not save completion history", err));
    }
  }
