...
// AIM CG v53a - warehouse operational job access, materials editing, stability cleanup
...
function App() {
  ...
  const currentRole = currentProfile?.role === "admin" ? "admin" : currentProfile?.role === "warehouse" ? "warehouse" : "employee";
  const isAdminUser = currentRole === "admin" && currentProfile?.active !== false;
  const isWarehouseUser = currentRole === "warehouse" && currentProfile?.active !== false;
  const canViewAdminCalendar = isAdminUser || isWarehouseUser;
  ...
  const hasStoresPermission = isAdminUser || isWarehouseUser || Boolean(linkedCurrentWorker?.storesPermission);
  const activeView = isWarehouseUser ? "admin" : isAdminUser ? view : "employee";
  ...
  useEffect(() => {
    ...
    if (canViewAdminCalendar) {
      setView("admin");
    } else if (view !== "employee") {
      setView("employee");
    }
  }, [currentUser?.id, profileLoading, canViewAdminCalendar]);
...
function getDraggedJobId(e) {
  ...
  async function saveJob(jobToSave) {
    const existingJob = dataRef.current.jobs.find(j => j.id === jobToSave.id);
    const exists = Boolean(existingJob);
    let next = normaliseJob(jobToSave);

    // Warehouse can update job material fields only. Scheduling, assignments,
    // commercial fields and general job details remain protected.
    if (isWarehouseUser) {
      if (!exists) {
        alert("Warehouse users cannot create jobs.");
        return false;
      }
      next = normaliseJob({
        ...existingJob,
        materialsStatus: next.materialsStatus,
        materials: next.materials
      });
      if (session && supabase) {
        try {
          setJobsSyncMessage("Saving job materials to Supabase...");
          await saveWarehouseJobMaterialsToSupabase(next);
          updateData(current => ({ ...current, jobs: current.jobs.map(j => j.id === next.id ? next : j) }));
          setJobsSyncMessage(`Saved materials for ${next.title || "job"}.`);
          setEditingJob(null);
          return true;
        } catch (err) {
          console.error(err);
          setJobsSyncMessage(err?.message ? `Warehouse job update failed: ${err.message}` : "Warehouse job update failed.");
          alert(err?.message || "Could not save job materials. The job editor has been left open so you can retry.");
          return false;
        }
      }
      updateData(current => ({ ...current, jobs: current.jobs.map(j => j.id === next.id ? next : j) }));
  ...
  async function saveWorkers(workers, deletedWorkerIds = []) {
    ...
    try {
      ...
      await syncWorkerRolesToSupabase(savedWorkers);
...
Reply: ${messageText}` };
  ...
  return (
    <div className={activeView === "admin" ? "app admin-mode" : currentUser ? "app nav-mode employee-mode" : "app"}>
      ...
      {!currentUser ? (
      ...
      ) : reportsOpen && (isAdminUser || isWarehouseUser) ? (
        <ReportsPage jobs={data.jobs} workers={data.teamMembers} inventoryItems={inventoryItems} inventoryLocations={inventoryLocations} inventoryMovements={inventoryMovements} stockOnly={isWarehouseUser} onClose={()=>setReportsOpen(false)} />
        ...
        <InventoryPage items={inventoryItems} locations={inventoryLocations} movements={inventoryMovements} jobs={data.jobs} workers={data.teamMembers} isAdmin={isAdminUser || isWarehouseUser} currentUser={currentUser} onClose={()=>setInventoryOpen(false)} onRefresh={refreshInventoryData} />
        ...
        <>
          {isAdminUser && <section className="admin-view-tabs">
          ...
          {isAdminUser && adminTab === "attention" ? (
          ...
          <main className="workspace">
            <aside className={`bucket-panel ${bucketsCollapsed ? "collapsed" : ""}`}>
              <div className="bucket-title"><PanelLeft size={18}/><div><h2>Job buckets</h2><span>{filteredJobs.length} jobs displayed{isWarehouseUser ? " · read only" : ""}</span></div><button type="button" className="bucket-collapse-button" onClick={()=>setBucketsCollapsed(v=>!v)} aria-label={bucketsCollapsed ? "Expand job buckets" : "Collapse job buckets"}>{bucketsCollapsed ? <ChevronDown size={18}/> : <ChevronUp size={18}/>}</button></div>
              ...
              <div className="bucket-button-grid">{["All",...CATEGORIES].map(cat=>{const count=cat==="All"?bucketJobs.length:bucketJobs.filter(j=>j.category===cat).length;return <button key={cat} className={activeCategory===cat?"active":""} onClick={()=>setActiveCategory(cat)} onDragOver={e=>{if(isAdminUser)e.preventDefault();}} onDrop={e=>{if(!isAdminUser)return;e.preventDefault(); const id=getDraggedJobId(e); if(id) moveToBucket(id,cat);}}><span>{cat}</span><em>{count}</em></button>})}</div>
              <div className="selected-bucket-list">{filteredJobs.map(job=><JobCard key={job.id} job={job} workerNames={getAssignedWorkerNames(data.teamMembers, job.assignedTo)} selected={selectedBucketJobId===job.id} onSelect={()=>setSelectedBucketJobId(job.id)} onDragStart={e=>{if(isAdminUser)handleDragStart(e,job.id)}} onEdit={()=>{if(isAdminUser||isWarehouseUser)setEditingJob(job)}} onDelete={()=>{if(isAdminUser)cancelOrDeleteJob(job)}} readOnly={isWarehouseUser}/>) }{filteredJobs.length===0 && <div className="empty small">No jobs in this bucket</div>}</div>
              </div>
            </aside>
      ...
      {settingsOpen && currentUser && <SettingsModal currentUser={currentUser} currentRole={currentRole} isAdminUser={isAdminUser} isWarehouseUser={isWarehouseUser} onManagePeople={()=>{setSettingsOpen(false);setPeopleOpen(true);}} onManageReferences={()=>{setSettingsOpen(false);setReferenceSettingsOpen(true);}} activeView={activeView} isInstalledPwa={isInstalledPwa} canPromptInstall={Boolean(installPrompt)} onInstall={installJobsched} onOpenCloseouts={()=>{setSettingsOpen(false);setToolsOpen(false);setInventoryOpen(false);setMachineryOpen(false);setReportsOpen(false);setJobPacksOpen(false);setCloseoutsOpen(true);}} onOpenJobPacks={()=>{setSettingsOpen(false);setToolsOpen(false);setInventoryOpen(false);setMachineryOpen(false);setReportsOpen(false);setCloseoutsOpen(false);setJobPacksOpen(true);}} onClose={()=>setSettingsOpen(false)} onSetView={(nextView)=>{setView(nextView); setSettingsOpen(false);}} onChangePassword={()=>{setPasswordSetupMode("manual"); setSettingsOpen(false);}} onSignOut={async()=>{setSettingsOpen(false); await signOut();}} />}
...
function CalendarGrid({ days, dayMin = "230px", workers, jobs, leaveRecords, messages, selectedJobId, selectedBooking, copiedBooking, onCellPasteBooking, onCellSelectBooking, onDropJob, onDragStart, onSelectBooking, onOpenJobMessages, onAddItem, onEditJob, onDeleteJob, onToggle, onConfirmComplete, onPreviousWeek, onNextWeek, readOnly = false, allowReadOnlyOpen = false }) {
...
function CalendarJob({ job, workerId, date, isStart, selected, hasAnyMessage, hasUnreadMessage, onSelect, onOpenMessages, onDragStart, onEdit, onDelete, onToggleAppointmentSent, onToggleClientAccepted, onConfirmComplete, readOnly = false, allowReadOnlyOpen = false }) {
  ...
  return (
    <article className={`calendar-job ${selected ? "selected-booking" : ""} ${defectJob ? "defect-callback" : ""} ${job.category === "Completed" || job.completedConfirmed ? "supervisor-complete" : ""} ${completedByTrade && job.category !== "Completed" ? "trade-complete" : ""} ${job.isAdHoc?"ad-hoc":""} ${job.isTravelComment?"travel-comment":""} ${isStart?"range-start":"range-middle"}`} draggable={!readOnly} onDragStart={readOnly?undefined:onDragStart} onClick={readOnly?(allowReadOnlyOpen?onEdit:undefined):onSelect}>
      <div className="range-body">
        {hasAnyMessage && !readOnly && <button type="button" className={hasUnreadMessage ? "job-envelope unread" : "job-envelope sent"} title={hasUnreadMessage ? "New message needs action" : "Sent/actioned messages"} onClick={(e)=>{e.stopPropagation(); onOpenMessages();}}><Mail size={13}/></button>}
        {isStart ? (
          <>
            <div className="card-top">
              ...
              {!readOnly&&<div className="card-actions"><button onClick={(e)=>{e.stopPropagation(); onEdit();}} title="Edit job"><Pencil size={14}/></button></div>}
            </div>
            <h3>{job.title}</h3>
...
function JobCard({ job, workerNames, selected, onSelect, onDragStart, onEdit, onDelete, readOnly = false }) {
  return (
    <article className={`card compact bucket-card-minimal ${selected ? "selected" : ""} ${isDefectJob(job) ? "defect-card" : ""}`} draggable={!readOnly} onClick={onSelect} onDragStart={readOnly?undefined:onDragStart} onDoubleClick={readOnly?undefined:onEdit}>
      <div className="card-top">
        <span className={`category-pill ${isDefectJob(job) ? "defect" : ""}`}>{isDefectJob(job) ? "Call back - Defects" : job.category}</span>
        <div className="card-actions">
          {readOnly?<button title="Open job materials" onClick={(e)=>{e.stopPropagation(); onEdit();}}><PackagePlus size={14}/></button>:<>
...
function JobModal({ job, teamMembers, tradeOptions = TRADES, siteOptions = BASE_SITES, tagOptions = [], onCreateTag = async()=>{}, currentUser, actorName = CURRENT_USER, isAdmin = false, warehouseMode = false, messages = [], machines = [], machineryBookings = [], inventoryItems = [], inventoryLocations = [], inventoryMovements = [], onInventoryRefresh = async()=>{}, onClose, onSave, onSendMessage, onActionMessage, onReplyMessage }) {
  ...
  const tabs = warehouseMode
    ? ["details","notes","materials","attachments","history"]
    : form.isAdHoc
  ...
  async function submit(e){
    ...
    if(warehouseMode){
      setSavingJob(true);
      try{ await onSave(form); } finally { setSavingJob(false); }
      return;
    }
  ...
  {activeTab==="notes"&&<section className="job-tab-panel">{warehouseMode?<><p className="muted">Job notes are visible to Warehouse users but can only be changed by Admin or the applicable Trade workflow.</p><HistoryList items={form.noteHistory||[]} type="notes"/></>:<><div className="note-entry note-entry-stacked"><textarea rows="3" value={noteInput} onChange={e=>setNoteInput(e.target.value)} placeholder="Add note history entry..."/><label className="inline-check"><input type="checkbox" checked={showNoteInTradeView} onChange={e=>setShowNoteInTradeView(e.target.checked)}/>Show note in Trade View</label><button type="button" className="secondary" onClick={addNote}>Add note</button></div><HistoryList items={form.noteHistory||[]} type="notes" onToggleTrade={(id,value)=>setForm(cur=>({...cur,noteHistory:(cur.noteHistory||[]).map(n=>n.id===id?{...n,showInTradeView:value}:n)}))}/></>}</section>}
  ...
  {activeTab==="attachments"&&<section className="job-tab-panel">{warehouseMode?<><p className="muted">Attachments are available for stores and materials coordination. Warehouse users cannot add or remove job attachments.</p><div className="simple-list">{(form.attachments||[]).map(a=><div key={a.id}><span>{a.name} · {formatBytes(a.size)}</span><button type="button" className="mini-action" onClick={()=>openStoredAttachment(a)}><Download size={14}/> Open</button></div>)}{!(form.attachments||[]).length&&<p>No attachments saved to this job.</p>}</div></>:<><div className="attachment-drop" onDragOver={e=>e.preventDefault()} onDrop={e=>{e.preventDefault(); addAttachments(e.dataTransfer.files||[]);}}><Paperclip size={24}/><strong>Drag files or photos here</strong><span>Files are uploaded to Supabase Storage and linked to this job.</span><label className="secondary file-pick">Choose files<input type="file" multiple disabled={attachmentUploading} onChange={e=>addAttachments(e.target.files||[])}/></label>{attachmentUploading && <span className="muted">Uploading...</span>}</div><div className="simple-list">{(form.attachments||[]).map(a=><div key={a.id}><span>{a.name} · {formatBytes(a.size)}</span><button type="button" className="mini-action" onClick={()=>openStoredAttachment(a)}><Download size={14}/> Open</button><button type="button" onClick={()=>setForm(cur=>({...cur,attachments:cur.attachments.filter(x=>x.id!==a.id)}))}><X size={14}/></button></div>)}</div></>}</section>}
  {activeTab==="history"&&<section className="job-tab-panel"><HistoryList items={form.jobHistory||[]} type="history"/></section>}
  <div className="modal-actions"><button type="button" className="secondary" onClick={onClose}>Cancel</button><button type="submit" className="primary" disabled={savingJob}>{savingJob?"Saving...":warehouseMode?"Save materials":"Save job"}</button></div></form></div>
}
...
function ReportsPage({ jobs, workers, inventoryItems = [], inventoryLocations = [], inventoryMovements = [], stockOnly = false, onClose }) {
  const [reportType, setReportType] = useState(stockOnly ? "inventory_stock" : "completed");
...
function SideNav({ unreadMessages, lowStockCount, isAdmin, isWarehouse, hasStoresPermission, onCalendar, onShare, onMessages, onReports, onCloseouts, onJobPacks, onTools, onInventory, onMachinery, onSettings }) {
  return (
    <nav className="side-nav" aria-label="Main actions">
      ...
      {(isAdmin || isWarehouse) && <button title={isWarehouse ? "Stock reports" : "Reports"} onClick={onReports}><BookOpen size={22}/></button>}
...
function SettingsModal({ currentUser, currentRole, isAdminUser, isWarehouseUser, onManagePeople, onManageReferences, activeView, isInstalledPwa, canPromptInstall, onInstall, onOpenCloseouts, onOpenJobPacks, onClose, onSetView, onChangePassword, onSignOut }) {
  return (
    <div className="modal-backdrop">
      <div className="modal mini-modal settings-menu-modal">
        ...
        <div className="settings-action-list">
          ...
          {isWarehouseUser && <button type="button" className="choice-card active"><Warehouse/><strong>Warehouse View</strong><span>Read-only schedule with full inventory access.</span></button>}
          {!isWarehouseUser && <button type="button" className={activeView === "employee" ? "choice-card active" : "choice-card"} onClick={() => onSetView("employee")}><Users/><strong>Trade View</strong><span>Assigned work, completion updates and tools.</span></button>}
        ...
        <div className="people-layout">
          ...
          {selected && <section className="worker-editor">
            ...
            <section className="access-panel">
              ...
              <p>Admin has full access. Warehouse has full inventory access plus a read-only Admin calendar. Employee uses the assigned Trade View.</p>
              <label>Permission level
                <select value={selected.appRole || "employee"} onChange={e=>update(selected.id,"appRole",e.target.value)}>
                  <option value="employee">Employee — assigned Trade View and job updates</option>
                  <option value="warehouse">Warehouse — full inventory + read-only Admin calendar</option>
                  <option value="admin">Admin — full read/write access</option>
                </select>
              </label>
              <label className="check-option plain"><input type="checkbox" checked={selected.appRole==="admin"||selected.appRole==="warehouse"||Boolean(selected.storesPermission)} disabled={selected.appRole==="admin"||selected.appRole==="warehouse"} onChange={e=>update(selected.id,"storesPermission",e.target.checked)}/>Stores permission</label>
...
function MachineryPage({machines,bookings,jobs,workers,days,onClose,onAddBooking,onEditBooking,onRefresh,onManageMachinery,onSaveMachine,onDeleteMachine}){const [selected,setSelected]=useState(null);const [machineryWeekStart,setMachineryWeekStart]=useState(()=>getStartOfWeek(new Date()));const shownDays=Array.from({length:Math.max(1,days.length)},(_,i)=>addDays(machineryWeekStart,i));return <main className="inventory-page machinery-page"><section className="inventory-shell"><div className="inventory-header machinery-page-header"><div><h1><Tractor size={28}/> Machinery</h1><p>All active machines on one calendar.</p></div><div className="machinery-toolbar-actions"><button className="secondary" onClick={()=>setMachineryWeekStart(d=>addDays(d,-7))}><ChevronLeft size={15}/> Previous week</button><button className="secondary" onClick={()=>setMachineryWeekStart(d=>addDays(d,7))}>Next week <ChevronRight size={15}/></button><button className="secondary" onClick={()=>setMachineryWeekStart(getStartOfWeek(new Date()))}>This week</button><button className="secondary" onClick={onRefresh}><RotateCcw size={15}/> Refresh</button><button className="secondary" onClick={onManageMachinery}><Settings size={15}/> Manage machinery</button><button className="primary" onClick={()=>onAddBooking({date:getIsoDate(new Date())})}><Plus size={15}/> Booking</button><button className="secondary machinery-back-button" onClick={onClose}><ChevronLeft size={17}/> Back</button></div></div><MachineryCalendar machines={machines.filter(m=>m.active!==false&&m.status!=="inactive")} bookings={bookings} jobs={jobs} workers={workers} days={shownDays} onAddBooking={onAddBooking} onEditBooking={onEditBooking} onEditMachine={setSelected}/></section>{selected&&<MachineryDetailModal machine={selected} bookings={bookings.filter(b=>b.machineId===selected.id)} jobs={jobs} workers={workers} onClose={()=>setSelected(null)} onSave={async m=>{await onSaveMachine(m);setSelected(null);}} onDelete={async id=>{if(confirm("Delete this machine and its history?")){await onDeleteMachine(id);setSelected(null);}}}/>}</main>}
...
async function syncWorkerRolesToSupabase(workers) {
  if (!supabase) throw new Error("Supabase is not configured for this build.");
  for (const worker of (workers || []).filter(w => isUuid(w.id))) {
    const role = worker.appRole === "admin" ? "admin" : worker.appRole === "warehouse" ? "warehouse" : "employee";
    const { error } = await supabase.rpc("aimcg_set_worker_app_role", { p_worker_id: worker.id, p_role: role });
    if (error) {
      if (["42883","PGRST202"].includes(error.code) || /aimcg_set_worker_app_role|function.*not found/i.test(error.message || "")) {
        throw new Error("Run SUPABASE_MIGRATION_v53_WAREHOUSE_ROLE_STABILITY.sql before saving Warehouse permissions.");
      }
      throw error;
    }
  }
...
async function saveWarehouseJobMaterialsToSupabase(job) {
  if (!supabase || !isUuid(job?.id)) throw new Error("A saved Supabase job is required.");
  const { error } = await supabase.rpc("aimcg_warehouse_update_job_materials", {
    p_job_id: job.id,
    p_materials_status: job.materialsStatus || "Parts from stock",
    p_materials: Array.isArray(job.materials) ? job.materials : []
  });
  if (error) {
    const missing = ["42883", "PGRST202", "PGRST205"].includes(error.code) || /aimcg_warehouse_update_job_materials|function.*not found/i.test(error.message || "");
    if (missing) throw new Error("Run SUPABASE_MIGRATION_v53a_WAREHOUSE_JOB_MATERIALS.sql before saving Warehouse job materials.");
    throw error;
  }
}
