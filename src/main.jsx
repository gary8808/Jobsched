...
function App() {
    ...
    async function checkSupabase() {
      ...
      try {
        ...
        if (workers.length) {
          ...
          if (!workers.some(w => w.id === employeeId)) setEmployeeId(workers[0].id);
        }
  ...
  return (
    <div className={view === "admin" ? "app admin-mode" : "app"}>
      ...
      {peopleOpen && <PeopleModal workers={data.teamMembers} usingSupabase={Boolean(session && supabase)} saving={workersLoading} onClose={()=>setPeopleOpen(false)} onSave={async (workers)=>{try{await saveWorkers(workers); setPeopleOpen(false);}catch(err){alert(err?.message || "Could not save employees to Supabase.");}}} />}
...
function PeopleModal({ workers, usingSupabase, saving, onClose, onSave }) {
  ...
  function deactivate(worker) {
    ...
    setItems(current => current.map(w => w.id === worker.id ? { ...w, inactive: true, accessRevoked: revoke } : w));
      ...
      <div className="modal settings-modal">
        ...
        <div className="choice-grid two-choice">
          <button type="button" className="choice-card" onClick={add}>
            ...
            <span>Create a new worker record in Supabase.</span>
          </button>
        ...
        <div className="people-layout">
          <aside className="people-list">
            ...
            {filtered.map(w=><button key={w.id} type="button" className={selected?.id===w.id?"active":""} onClick={()=>setSelectedId(w.id)}><strong>{w.name||"Unnamed employee"}</strong><span>{w.trade||"No trade"}{w.inactive ? " · inactive" : ""}</span></button>)}
            {!filtered.length && <p className="muted">No employees found.</p>}
          </aside>
          ...
          {selected && <section className="worker-editor">
            ...
            <label className="check-option plain"><input type="checkbox" checked={!!selected.inactive} onChange={e=>e.target.checked ? deactivate(selected) : update(selected.id,"inactive",false)}/>Deactivate employee</label>
        ...
        <div className="modal-actions">
          ...
          <button className="primary" onClick={()=>onSave(items)} disabled={saving}>{saving ? "Saving..." : "Save people"}</button>
        </div>
      </div>
    </div>
  );
...
async function fetchWorkersFromSupabase() {
  ...
  const { data, error } = await supabase
    .from("workers")
    .select("id,profile_id,name,trade,base_site,phone,email,sap_number,birthday,roster_pattern,roster_start_date,inactive,access_revoked,created_at,updated_at")
