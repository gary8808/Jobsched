...
Reply: ${messageText}` };
  ...
  return (
    <div className={activeView === "admin" ? "app admin-mode" : currentUser ? "app nav-mode employee-mode" : "app"}>
      {currentUser && <SideNav unreadMessages={unreadMessages} lowStockCount={lowStockCount} isAdmin={isAdminUser} hasStoresPermission={hasStoresPermission} onCalendar={() => { setToolsOpen(false); setInventoryOpen(false); setMachineryOpen(false); setReportsOpen(false); if (isAdminUser) setAdminTab("schedule"); }} onShare={() => setShareHubOpen(true)} onMessages={() => setMessagesOpen(true)} onPeople={() => setPeopleOpen(true)} onReports={() => {setToolsOpen(false);setInventoryOpen(false);setMachineryOpen(false);setReportsOpen(true);}} onTools={() => {setReportsOpen(false);setInventoryOpen(false);setMachineryOpen(false);setToolsOpen(true);}} onInventory={() => {setReportsOpen(false);setToolsOpen(false);setMachineryOpen(false);setInventoryOpen(true);}} onMachinery={() => {setReportsOpen(false);setToolsOpen(false);setInventoryOpen(false);setMachineryOpen(true);}} onSettings={() => setSettingsOpen(true)} />}
