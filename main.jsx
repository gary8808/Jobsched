...
const STORAGE_KEY = "jobsched-v15-flexible-bookings";
...
function App() {
  ...
  const [dashboardTab, setDashboardTab] = useState("Not ready");
...
function NeedsAttentionView({ jobs, workers, days, leaveRecords, messages, activeTab, setActiveTab, onOpenJob }) {
  const activeJobs = jobs.filter(j => j.category !== "Cancelled" && j.category !== "Completed");
  const notReady = activeJobs.filter(job => !getReadiness(job).ready);
  ...
  const groups = {
    "Not ready": { jobs: notReady, hint: "Missing key scheduling information" },
  ...
  const selected = groups[activeTab] || groups["Not ready"];
