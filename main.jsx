                ...
                return (
                  <article key={job.id} className="employee-job-card app-job-card">
                    ...
                    <div className="employee-quick-info">
                      ...
                      <span>{job.requiredTrade || "No trade set"}</span>
                      <span>Materials: {job.materialsStatus || "Not checked"}</span>
                    </div>

                    <div className="employee-actions primary-actions-row">
                      <a className="secondary" href={job.clientPhone ? `tel:${job.clientPhone}` : undefined} onClick={(e) => { if (!job.clientPhone) { e.preventDefault(); alert("No client phone number saved."); } }}><Phone size={16}/> Call</a>
                      <button className="secondary" onClick={() => onStatus(job.id, worker.id, "running")}><Play size={16}/> Start</button>
                      <button className="secondary" onClick={() => onStatus(job.id, worker.id, "paused")}><Pause size={16}/> Pause</button>
                      <button className="secondary" onClick={() => onStatus(job.id, worker.id, "stopped")}><Square size={16}/> Stop</button>
